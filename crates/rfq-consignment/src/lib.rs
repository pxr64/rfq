...
//! sell/buy gates, the broker pre-check, and the SPV prover (Phase 1 of
//! `docs/consignment-validation-hardening-plan.md`).
//!
//! First component: the pipelined, multi-endpoint **mined-witness checker** — the
//! perf-critical, chain-only "is every witness mined?" check that both the sell-side
//! gate (all witnesses must be mined) and the buy-side gate (all *except the swap tx*)
//! perform.
//!
...
//!
//! This crate is electrum-only (no esplora/Core dependency), per the project decision.
//!
//! NOTE: the maker/taker gates currently live in `rfq-rgb` (`validate_incoming_consignment`
//! / `validate_buy_consignment`) using rgb-ops' serial resolver. Migrating them onto this
//! checker (extract the witness-txid set, then one `MinedChecker::check`) is the next step
//! — kept separate so the working gates aren't disturbed.
