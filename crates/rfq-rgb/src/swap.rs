...
use bpstd::{
    ...
    TxOut as BpTxOut, TxVer, Txid, VarIntArray, Vout, XprivAccount, XpubDerivable,
};
use bpwallet::Wallet;
...
pub(crate) fn assemble_unsigned_psbt(
    ...
    let mut tx_outputs = vec![BpTxOut::new(
        inputs.maker_payout_spk.clone(),
        Sats(gross_btc_sats),
    )];
...
pub(crate) fn encode_swap_transfer_sell(
    ...
    let consignment = match change_consignment {
        Some(transfer) => {
            ...
            transfer.save(&mut bytes).map_err(|e| {
                RgbError::TransferBuild(format!("serialize change consignment: {e}"))
            })?;
...
fn resolve_maker_btc_inputs(
    ...
    for (want, _txout) in maker_btc_inputs {
        ...
        let script = spk_by_outpoint
            ...
            .ok_or_else(|| {
                RgbError::TransferBuild(format!(
                    "no cached scriptPubkey for maker BTC outpoint {want}"
                ))
            })?;
...
/// Persist the tapret tweak `rgb_commit` wrote onto the host output so the maker's
/// wallet still recognises the (tweaked) host as its own. Factored out of
/// [`commit_and_consign`] so the unilateral self-send paths can reuse it. Shared
/// with the recovery/rebalance submodule via `super::`.
pub(super) fn persist_tapret_tweak(wallet: &mut MakerWallet, psbt: &mut Psbt) {
